abstract class CharacterSheet {
    ...
    static CULTURES: string[] = ['american'];
    ...
    generateBackgrounds(points) {
        ...
        while (points > 0) {
            if (backgroundsLessThan5.length > 0 && Math.random() * 100 < this.options.chanceToRepeatBackgrounds) {
                ...
                if (this.stats.backgrounds[this.getBackgroundByName(background)].value == 4 || this.stats.backgrounds[this.getBackgroundByName(background)].name == 'Avatar') background = backgroundsLessThan5.sample();
            }
            else {
	 ...
	 */
    changeBackground(name: string, points: number) {
        var backgroundIndex = this.getBackgroundByName(name);

        if (backgroundIndex >= 0) {
            this.stats.backgrounds[backgroundIndex].value += points;

            if (this.stats.backgrounds[backgroundIndex].value > 5) this.stats.backgrounds[backgroundIndex].value = 5;
	 ...
	 */
    decreaseRandomAttributes(possibleAttributes, points) {
    ...
    generateMeritsAndFlaws(min, max) {
    ...
    addMerit(merit) {
        if (!this.stats.merits.any(function (m) { return m.name == merit.name; })) {
            if (!merit.hasOwnProperty('mayAdd') || merit.mayAdd(this)) {
                ...
                if (merit.hasOwnProperty('onAdd')) merit.onAdd(this);
            }
        }
    };
    ...
    addFlaw(flaw) {
        if (!this.stats.flaws.any(function (f) { return f.name == flaw.name; })) {
            if (!flaw.hasOwnProperty('mayAdd') || flaw.mayAdd(this)) {
                ...
                if (flaw.hasOwnProperty('onAdd')) flaw.onAdd(this);
            }
        }
    };

    hasMeritOrFlaw(meritOrFlawName) {
    ...
    generateWillpower(max: number, chancePerIncrease: number) {
        for (var i = 0; i < max; i++) {
	 ...
	 */
    generateAttributes(primary: number, secondary: number, tertiary: number) {
    ...
    assignAttributePoints(possibleAttributes, points: number) {
        var attribute;
        ...
        while (points > 0 && possibleAttributes.length > 0) {
            ...
            if (this.stats.attributes[attribute].value == 4) attribute = possibleAttributes.sample();
    ...
    generateAbilities(num: number) {
        ...
        for (i = 0; i < num; i++) {
            if (Math.random() * 100 < this.options.chanceToRepeatAbilities && existingAbilitiesLessThan5.length > 0) {
                ...
                if (this.stats.abilities[ability].value == 4) ability = existingAbilitiesLessThan5.sample();
            }
            else {
    ...
    changeAbility(ability, amount) {
        ...
        if (this.stats.abilities[ability].value > 5) this.stats.abilities[ability].value = 5;
        else if (this.stats.abilities[ability].value < 0) this.stats.abilities[ability].value = 0;
...
class MageCharacterSheet extends CharacterSheet {
    static TRADITIONS: string[] = [
        'Akashic Brotherhood',
        'Celestial Chorus',
        'Cult of Ecstasy',
        'Dreamspeakers',
